...
class TestEconomicRouter:
    ...
    def test_missing_telemetry_negative_inf(self):
        router = Economic()
        frag = make_fragment(reuse_score=1.0)
        telemetry = {"n1": Telemetry("n1", 0.0, 0.0, 0.0, 0.0)}
        best = router.route(frag, ["n1", "n2"], telemetry, [])
        assert best == "n1"
    ...
    def test_value_density_affects_choice(self):
        ...
        telemetry = {
            "n1": Telemetry("n1", 0.0, 0.0, 0.0, 0.0),
        }
    ...
    def test_latency_normalization_prevents_domination(self):
        ...
        router = Economic(config=EconomicRouterConfig(max_latency_ms=1000.0))
        ...
        telemetry = {
            "n1": Telemetry("n1", 5000.0, 0.0, 0.0, 0.0),
            "n2": Telemetry("n2", 100.0, 0.5, 0.5, 0.5),
        }
        # n1 cost = 1.0 * 1.0 = 1.0
    ...
    def test_weighted_config_changes_outcome(self):
        ...
        frag = make_fragment(reuse_score=0.5)
        telemetry = {
            "fast_but_overloaded": Telemetry("fast_but_overloaded", 10.0, 0.0, 0.9, 0.9),
            "slow_but_idle": Telemetry("slow_but_idle", 5000.0, 0.0, 0.1, 0.1),
        }
        # With default weights, fast_but_overloaded should lose because of
        # high gpu/memory cost.
        router_default = Economic()
    ...
    def test_clamped_values_stay_in_range(self):
        ...
        frag = make_fragment()
        telemetry = {
            "n1": Telemetry("n1", 100_000.0, 5.0, 2.0, -1.0),
        }
