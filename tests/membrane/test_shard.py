...
from membrane.ring import EmptyRingError, Ring
from membrane.shard import Shard


class TestShardManager:
    """Test suite for Shard."""
    ...
    def test_get_replicas(self):
        ...
        ring.add_node("n2")
        ring.add_node("n3")
        sm = Shard(hash_ring=ring, replica_count=2)
    ...
    def test_is_replica(self):
        ...
        ring.add_node("n1")
        ring.add_node("n2")
        sm = Shard(hash_ring=ring, replica_count=1)
        sm.assign_shard("h1")
    ...
    def test_replica_count_zero(self):
        ring = Ring()
        ring.add_node("n1")
        sm = Shard(hash_ring=ring, replica_count=0)
        replicas = sm.get_replicas("h1")
        assert replicas == set()
