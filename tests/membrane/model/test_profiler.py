...
def test_measured_points_exact():
    ...
    for length, expected_size, expected_time in zip(
        ...
        assert math.isclose(profiler.kv_size(length), expected_size, rel_tol=1e-9)
        assert math.isclose(profiler.prefill_time(length), expected_time, rel_tol=1e-9)
...
def test_interpolation_between_points():
    ...
    size = profiler.kv_size(length)
    time_s = profiler.prefill_time(length)
...
def test_clamping_below_minimum():
    ...
    assert profiler.kv_size(64) == 190.8
    assert profiler.prefill_time(64) == 0.44


def test_clamping_above_maximum():
    """Values above the maximum measured length should be clamped."""
    assert profiler.kv_size(200000) == 2316.3
    assert profiler.prefill_time(200000) == 7.40
...
def test_kv_throughput_consistency():
    ...
    for length, expected in zip(profiler.MEASURED_LENGTHS, profiler.MEASURED_KV_THROUGHPUTS):
        computed = profiler.kv_throughput(length)
