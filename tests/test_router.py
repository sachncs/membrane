...
def test_long_request_routed_to_membrane():
    ...
    rtr = router.Router(threshold=1000)
    decision = rtr.route(total_length=1200, cached_prefix_membrane=0, cached_prefix_pd=0)
...
def test_bandwidth_scarce_independent_cache():
    ...
    decision = rtr.route(total_length=2000, cached_prefix_membrane=0, cached_prefix_pd=1500)
    assert decision.target == "pd-p"
...
def test_bandwidth_abundant_best_cache():
    ...
    decision = rtr.route(total_length=2000, cached_prefix_membrane=1800, cached_prefix_pd=0)
    assert decision.target == "pd-p"
