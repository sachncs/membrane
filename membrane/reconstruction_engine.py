...
class ReconstructionResult:
    ...
    Attributes:
        fragments: Assembled fragment chain covering the prompt.
        coverage_ratio: Fraction of prompt tokens covered by fragments.
        missing_segments: Token ranges that required prefill fallback.
...
class ReconstructionConfig:
    ...
    Attributes:
        max_gap_tokens: Maximum uncovered gap before triggering prefill fallback.
        max_prefix_attempts: Max prefix lengths to scan for exact match.
    """
...
class ReconstructionEngine:
    ...
    def __init__(
        ...
        Args:
            index_system: In-memory indices for exact/semantic/positional lookup.
            prefill_adapter: Adapter for prefill fallback.
            config: Reconstruction thresholds. Defaults to max_gap_tokens=256.
        """
    ...
    def rebuild_context(
        ...
        Returns:
            ReconstructionResult with assembled fragments and metadata.
        """
        ...
        # Step 2: extend forward via positional index
        ...
        # Step 3: fill gaps via semantic index
            ...
            if len(remaining_tokens) > self.config.max_gap_tokens:
                ...
                if result.fragments:
                    ...
                    for frag in result.fragments:
                        ...
                        # Map fragment span to absolute prompt positions
        ...
        # Step 5: assemble and record co-access graph links
    ...
    def find_longest_exact_match(
        ...
        .. warning::
            This method computes an MD5 hash for every prefix length tried.
            For a prompt of length L, the worst-case cost is O(L²) in token
            count.  The *max_prefix_attempts* parameter caps the number of
            lengths scanned; if your prompts are longer than this cap, the
            method may miss an exact match that is shorter than the skipped
            lengths.

        Args:
            prompt_tokens: Input token IDs.
            model_id: Model identifier (also checked on candidate fragments).
            max_prefix_attempts: Maximum number of prefix lengths to try.
                Defaults to 128 to keep latency bounded.

        Returns:
            Longest matching fragment, or None if no exact match exists.
        """
        if max_prefix_attempts is None:
            max_prefix_attempts = self.config.max_prefix_attempts
        length = len(prompt_tokens)
        best: Fragment | None = None
        # Scan from longest to shortest, but cap the number of attempts.
    ...
    def find_next_adjacent(
        ...
        """Find a fragment adjacent to current_end via positional index.
        ...
        Returns:
            Adjacent fragment if found, else None.
        """
    ...
    def find_gaps(self, coverage: list[bool]) -> list[tuple[int, int]]:
        ...
        Returns:
            List of (start, end) inclusive ranges that are uncovered.
        """
    ...
    def covered_end(self, coverage: list[bool]) -> int:
        ...
        Returns:
            Rightmost covered index, or -1 if none covered.
        """
    ...
    def deduplicate_and_sort(self, fragments: list[Fragment]) -> list[Fragment]:
        ...
        Returns:
            Deduplicated fragments ordered by token span.
        """
        seen: set[str] = set()
    ...
    def record_graph_links(self, fragments: list[Fragment]) -> None:
        """Record co-access and positional edges between assembled fragments.

        Args:
            fragments: Ordered list of fragments from a reconstruction.
        """
